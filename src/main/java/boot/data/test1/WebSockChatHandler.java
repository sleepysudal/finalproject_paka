...
@Component
@Primary //웹소켓 관련 빈이 2개 잇어서 해당 것을 우선적으로 선택
public class WebSockChatHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        ChatMessageTest chatMessage = objectMapper.readValue(payload, ChatMessageTest.class);
        ChatRoomTest room = chatService.findRoomById(chatMessage.getRoomId());
        Set<WebSocketSession> sessions=room.getSessions();   //방에 있는 현재 사용자 한명이 WebsocketSession
        if (chatMessage.getType().equals(ChatMessageTest.MessageType.ENTER)) {
        ...
        }else if (chatMessage.getType().equals(ChatMessageTest.MessageType.QUIT)) {
